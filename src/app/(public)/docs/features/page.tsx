...
const FEATURES: FeatureCategory[] = [
  {
    ...
    items: [
      ...
      { name: 'Privacy levels (coming soon)', desc: 'Choosing between Public, Church Only and pastoral-staff-only visibility is not built. Every request submitted today goes to the public community feed, with your name attached unless you post anonymously.' },
  ...
  },
  {
    ...
    emoji: '⛪',
    subtitle: 'Pastoral tools, member management, and the church surfaces still being finished.',
    ...
    items: [
      ...
      { name: 'Church prayer wall (coming soon)', desc: 'The wall page is built and members-only, but a prayer is not attached to a church when it is submitted, so the wall stays empty. Nothing is gated behind a price — the church link simply is not written yet.' },
      { name: 'Member management', desc: 'Invite members, assign roles (admin, pastor, member), remove members.' },
      { name: 'Church groups (coming soon)', desc: "Members can start a prayer group today, but a group is not attached to a church, so the church Groups page is empty. Church-owned sub-groups (e.g. Women's Ministry, Youth) are not built yet." },
      { name: 'Pastoral dashboard', desc: 'Where pastoral notes and assignments live. Its prayer-activity counters read from the church link above, so they show zero until that is fixed.' },
      { name: 'Pastoral notes', desc: 'Pastors can add private notes to prayer requests for follow-up tracking.' },
      { name: 'Assignments', desc: 'Assign a prayer request to a specific pastor or leader for follow-up. There are no church prayers to assign yet.' },
      { name: 'Custom branding', desc: 'Upload a church logo and set accent colors for your church page and welcome message.' },
      { name: 'Analytics (not available yet)', desc: 'The analytics page is switched off. Four of its five charts read the church link above and drew a flat zero for every church, which reads as a false claim about a congregation. Member growth was the one real chart and it returns with the rest.' },
      { name: 'Prayer analytics (coming soon)', desc: 'Prayer volume, engagement, response rates and category breakdown all read from the church link above, so each shows zero. There is no analytics export.' },
      { name: 'Testimony review (not available yet)', desc: 'The approval queue is switched off. Nothing in the app ever submitted a testimony to it, so it could never fill.' },
      { name: 'Flagged prayers (not available yet)', desc: 'The moderation queue for flagged church prayers is switched off. Nothing in the app can flag a prayer yet, so it could never fill.' },
      { name: 'Live event prayer wall (coming soon)', desc: 'The projected, moderated wall for services, conferences and retreats is built, but there is no way to create an event, so it cannot be reached.' },
      { name: 'Event moderation (coming soon)', desc: 'Approve or reject prayers before they appear on the live event wall. Reachable only once events can be created.' },
      { name: 'Event display mode (coming soon)', desc: 'Full-screen, branded display for the live event wall. Reachable only once events can be created.' },
      { name: 'Event reports (coming soon)', desc: 'CSV export of all prayers submitted during an event. Reachable only once events can be created.' },
