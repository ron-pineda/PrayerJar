...
export default async function TestimonyQueuePage({ params, searchParams }: Props) {
  ...
  if (!session?.user?.id) {
    redirect(`/sign-in?callbackUrl=/church/${slug}/dashboard/testimony`);
  }
  ...
  if (!hasTestimonyApprovalQueue(tier)) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-muted-foreground mb-4">
          The Testimony Approval Queue is included on the {PLANS[TESTIMONY_APPROVAL_QUEUE_TIER].name} plan and
          above.
        </p>
