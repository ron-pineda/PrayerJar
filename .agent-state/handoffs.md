...
[2026-07-25] Analytics → QA: pj-s26-03 — Vercel Hobby collects NO custom events; the Sprint-17 funnel never produced a data point. PII leak closed (prayer/testimony IDs were in the raw analytics path). Migration 0033 written, NOT applied.
[2026-07-25] Growth → QA: pj-s26-04/05 — /api/og/card/* returns 500 in PROD (every shared prayer link has no social card); /pray/[category] accepted any slug at 200; 2 false "thousands" claims removed from meta descriptions.
[2026-07-25] Growth+Content → PM: pj-s26-06 — refused the paid church pitch; shipped a free-tier invite instead. Surfaced that the church product is substantially non-functional.
[2026-07-25] PM → Team: pj-s26-09/10/11/12 created. Ron's direction: stop selling the church tier, then fix it. pj-s26-12 is destructive and blocked on Ron.
...
[2026-07-25] Reviewer → done: pj-s26-01, pj-s26-02, pj-s26-03, pj-s26-04, pj-s26-05 — approved. Gates re-run independently on f86e963, not accepted from report: tsc exit 0; vitest 64 files / 513 tests passing; next build exit 0. pj-s26-02 approved only after spot-checking three of its own claims (ADMIN_EMAILS at admin-auth.ts:11-15 and :34-38; sprints.json 52/51-done/19-unknown; deploy.yml push trigger) — a reconciliation task cannot be approved on its own report.
[2026-07-25] Reviewer → Architect: pj-s26-06-church-acquisition-asset — REWORK: docs/sprint26/church-outreach-asset.md:88 tells a pastor "nothing is public to search engines" about member prayer content. False — robots.ts is allow:"/", p/[id]/page.tsx:34,37,43 renders prayer.content into three meta description fields, and this sprint's own seo-audit.md:234 confirms there is no noindex anywhere. Privacy claim, in the artefact Ron hands over, missed by the document's own 19-row claims table. One clause to fix. Risk: HIGH.
[2026-07-25] Reviewer → Architect: pj-s26-09-church-claims-honesty-pass — REWORK: src/app/(dashboard)/billing/page.tsx:114 still reads "Church features — private prayer walls and pastoral tools — are included on paid plans", present tense, directly above the upgrade button, in the file cee2439 edited for this exact reason. Same data-vs-hardcoded-string blind spot the task's own note names. One sentence to fix. Risk: MEDIUM-HIGH.
[2026-07-25] Reviewer → PM: NOT PUSHED — two tasks at needs-rework and push to feature/prayer-jar auto-deploys to production. Sprint 26 does not close this session. Full sign-off: docs/sprint26/reviewer-signoff.md.
[2026-07-25] Reviewer → PM: three corrections to the ledger. (1) 1f4dc8f is NOT misattributed — one file, matching its own message; only c386164 is contaminated (13 files pj-s26-04/05 + 17 files pj-s26-03). The 2026-07-25 PM note above needs amending. (2) The open decision "migration 0033 must be applied BEFORE next deploy" is wrong twice — deploy.yml already runs db:migrate:deploy before publishing and fails closed, so no hand-run is needed; and the consequence of a missed migration is an AUTH OUTAGE (drizzle bare select() emits the six new columns; @auth/drizzle-adapter uses bare select() on users), not "signups lose attribution". (3) Trust floors 5/25/50 are unratified policy — INBOX item, does not block; keep them hardcoded, an env-driven trust threshold would let a prod env change switch social proof on with no diff.
[2026-07-25] Reviewer → Architect: pj-s26-09 addendum — second survivor in the same rework: church/join/page.tsx:122-124 ("Head over to your church's prayer wall to see what's been shared") links to the wall whose empty state this same task rewrote to "This wall is not receiving prayers yet". Not an independent blocker; fix with /billing:114.
[2026-07-25] Reviewer → PM: pj-s26-07 (QA gate pass) never ran and is still `proposed` — this review is the only verification Sprint 26 received. Mechanical gates re-run by me (build verbatim 108/108); the live half of -07 ("/docs copy fix verified on the deployed site") is unperformable until push and must be closed after deploy alongside pj-s26-11. Also disclosed: the two subagent passes Reviewer.md mandates were not run (operating instructions forbid unrequested subagents); reviewed by hand instead.
...
[2026-07-25] Frontend+Backend → QA: pj-s27-02-pull-paid-tiers — paid church tiers removed from the product; no price, tier name, comparison or upgrade CTA remains on any user-facing surface. Commit 0d06aea. Gates: tsc 0, vitest 64/501 passing, next build 106/106 (was 108 — /docs/paid and /for-churches/demo deleted, both 308 redirected).
[2026-07-25] Frontend+Backend → QA: pj-s27-03-free-limits — free limits now { members: null, groups: null, events: 0, admins: 999 }. Member cap deleted from addChurchMember; events: 0 retained deliberately as a feature lock with a plan-free message. Same commit 0d06aea.
[2026-07-25] Frontend+Backend → Architect (for §E task 8 ADRs, NOT done here): record in decisions.md — (1) the dormant plans.ts tier predicates, (2) the getChurchTier vs churches.current_plan source-of-truth split (C.6), (3) the retained hasCustomSubdomain gate (C.5).
[2026-07-25] Frontend+Backend → PM (open question, deliberately unresolved): removal-plan §F.1. /billing is now a Giving page but still redirects non-admins to /profile, so an ordinary donor cannot see their own gifts. The role check's stated reason ("regular users never see plan tiers") no longer exists. Left in place because §F is out of scope; flagged for a decision.
