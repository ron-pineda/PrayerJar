...
## 2026-04-20 pj-s22-24-frontend-jar-swap Clay vessel replaces glass jar site-wide
Replaces the translucent glass jar with an earthen clay vessel per Designer spec (docs/design/clay-vessel-spec.md). Adds 11 --clay-* OKLCH tokens (dark + light), @keyframes clay-overflow-pulse, prefers-reduced-motion extension, and a forced-colors media rule in globals.css. Rewrites src/components/prayer-jar.tsx as an amphora-silhouette vessel with mottling, throwing lines, inner highlight, 5-stop rim gradient, dark mouth ellipse, and overflow glow above the rim (opacity scales with count per spec §5; DOM absent at count===0). Retires all rgba(212,168,67,…) and rgba(10,10,20,0.8) literals. sm size grows 100×120 → 170×210; mode='slips' silently collapses to lights with @deprecated JSDoc; SlipDropAnimation export preserved. New src/components/prayer-jar-mark.tsx (24×24 two-path SVG, currentColor body, var(--primary) dot) swaps 🫙 at nav (layout.tsx), error boundary, and 404 page. Email header gets a 72×72 baked-fill PNG via new scripts/generate-jar-mark-png.mjs (reuses existing @resvg/resvg-wasm dep); PNG committed at both src/emails/assets/ and public/email-assets/. Spec deviation: --primary is stored as a full oklch() expression, so oklch(var(--primary)/α) nests invalidly — Frontend substituted color-mix(in oklch, var(--primary) X%, transparent) for alpha stops. Observation-level deviation: four --clay-* tokens (mottle-warm, mottle-cool, throwing, shadow) were stored as triplets without alpha, so per-theme alphas from spec §2 collapsed to a single constant at the consumption site (most visible: light-mode shadow renders at 0.40 instead of spec's 0.18); recorded as Sprint-23 follow-up. Visual regression unverifiable locally (build blocked on unrelated ADMIN_EMAILS env; dev server blocked by known Windows Sentry+Turbopack issue) — Vercel preview gates the final check. 8 consumer files of PrayerJar untouched; src/app/preview/jar-slips/ left intact for Ron to decide (§14.9). Reviewed and approved 2026-04-20.
...
## 2026-07-25 pj-s26-01-pdf-claims-fix Remove unbacked PDF-report claims from /docs
Drops the "Advanced analytics & PDF reports" ✓ from the Pro and Enterprise pricing columns of the docs/paid FEATURE_MATRIX and clears the PDF-export promises from docs/churches and docs/features, using the existing '(coming soon)' + 'Roadmap' honest-label convention. Verified no PDF implementation exists (no jspdf/pdfkit/puppeteer/react-pdf/pdf-lib in package.json or src; every export route is CSV or JSON); no exporter built. Also retitled two section headings whose wording re-implied the removed claim. Closes the last of the three Sprint 21-review Pro-tier claims not backed by code. Reviewed and approved 2026-07-25.

## 2026-07-25 pj-s26-02-state-reconciliation Reconcile sprints.json, ROADMAP.md and INBOX.md
Backfills sprints 23/24/25, never written at close, and reconciles 19 stale "planned" rows from the pre-launch 0.x-5.x roadmap that shipped under the numbered scheme — each closed as done with the shipped surface cited as evidence and closed_at:"unknown" so no date is invented. Ledger now 52 entries, 51 done / 1 planned. All three ROADMAP "queued" items verified individually as already shipped (GitHub Actions auto-deploy, custom email domain, multi-admin). INBOX: Better Stack log drain closed won't-do, subdomain DNS re-flagged at three months open, Sprint 22 Ron-leftovers added. BUILD_PROGRESS: two stale "pending" notes corrected. Reviewer spot-checked three claims independently rather than approving on the task's own report. Reviewed and approved 2026-07-25.

## 2026-07-25 pj-s26-03-funnel-instrumentation Instrument the acquisition funnel
Finds that Vercel Web Analytics on the Hobby plan collects no custom events, so the Sprint-17 funnel (12 track() wrappers, previously approved and marked done) has never produced a data point — and that Pro would not fix it (UTM is a Plus add-on; Pro caps custom events at 2 properties while trackPlanUpgraded passes 7). Closes a real PII leak: @vercel/analytics/next sent both the route and the raw path, so prayer and testimony IDs were leaving the browser; new src/components/web-analytics.tsx wraps <Analytics> with a beforeSend hook (src/lib/analytics-redact.ts) redacting those plus ?q= search terms and ?text= prayer content. Adds first-touch signup attribution: src/proxy.ts stamps a pj_attr cookie on an anonymous visitor's first HTML document, src/lib/auth.ts persists it in events.createUser inside its own try/catch, migration 0033 adds six nullable columns to users with a partial index and no backfill. Suite 459 → 513. Reviewed and approved 2026-07-25 with one correction: deploy.yml already applies 0033 before publishing and fails closed, so no hand-run is required, and the consequence of a missed migration is an auth outage rather than an attribution gap.

## 2026-07-25 pj-s26-04-seo-audit SEO & discoverability audit
Repairs /api/og/card/* returning 500 in production — every shared prayer link rendered with no social card — by moving all four OG route handlers from @vercel/og to next/og so the Satori/Resvg WASM is traced into the bundle. Does not reproduce locally, so it stays unproven until pj-s26-11 curls it against prod. Closes an open crawl space: /pray/[category] accepted any slug and returned 200 reflecting it into the title (/pray/cheap-viagra served "cheap-viagra Prayer Requests | PrayerJar"), now notFound(). Sitemap corrected — three hard-404 URLs removed (/testimony, /campaigns, /wrapped have no index route) and the ten real /pray/[category] pages added. Two false "thousands" claims removed from Google-facing meta descriptions. Deliverable: docs/sprint26/seo-audit.md. Reviewed and approved 2026-07-25, conditional on pj-s26-11 running immediately after deploy.

## 2026-07-25 pj-s26-05-share-invite-mechanics Share/invite path review
Documents what a real user can do today to bring a second user and where the path breaks. Clearest defect: /wrapped share tweets the raw PNG endpoint, so a click lands on a bare image with no path back to the site, and it publishes the user's DB ID — exact diff left for Frontend, not landed. Deliberately declines to unblock the robots.txt disallow on /church/: it is a shared prefix covering member walls, dashboards and admin, so opening it for public profiles risks the gated siblings for zero upside at one church. Open question raised rather than guessed: the tweet hardcodes @PrayerJar and the handle could not be verified. Docs only. Deliverable: docs/sprint26/share-path-review.md. Reviewed and approved 2026-07-25.

## 2026-07-26 pj-s26-13-restore-wall Restore the expired prayers and suspend auto-expiry
Restores 11 real production prayers that had all reached status:'expired' — the 30-day auto-expiry kept running through the three-month email outage that stopped signups, so /browse told every visitor "Be the first to share a prayer" while real content sat hidden. Suspension uses two genuinely independent locks: the expire-prayers entry is removed from vercel.json, and the route no-ops unless PRAYER_EXPIRY_ENABLED is exactly 'true', returning an explanatory JSON body rather than a silent 200. Either alone is sufficient and neither fails the way the other does. scripts/restore-expired-prayers.mjs is dry-run by default, touches only status='expired', asserts the answered count is unchanged and exits 1 if it moved, and sets expiresAt a year out so a future re-enable cannot wipe the restored rows. Adds 4 tests for a cron gate that previously had none. Reviewed and approved 2026-07-26, with the re-enable hazard opened as pj-s27-10 rather than rejected: expireOverduePrayers is an unbounded UPDATE with no batch cap while createPrayer still stamps expiresAt 30 days out, so the population one env-var flip would hide grows every week, guarded only by a code comment.

## 2026-07-26 pj-s27-03-free-limits Make the free tier not dead-end
Free limits become { members: null, groups: null, events: 0, admins: 999 }. The member cap was deleted from addChurchMember rather than raised — it threw at member 76 and the person who saw the upsell was the joining congregant, a stranger to the billing relationship, with nothing on the other side to upgrade to. groups was never enforced anywhere; admins was never read at all and is typed number, so 999 rather than null. events: 0 is retained deliberately as a feature lock, not a price: POST /api/v1/church/[slug]/events is live and admin-reachable while live events have no create, edit or delete UI, so that throw is the only thing keeping an unfinished surface shut; only its message changed, and a test guards that it names no plan. Tests were inverted into regression guards rather than deleted — plans.test.ts now asserts the caps are null and that the features list quotes no member or group number, and church-platform.service.test.ts fails if a cap is ever reintroduced. Reviewed and approved 2026-07-26.

## 2026-07-26 pj-s27-02-pull-paid-tiers Remove the paid church tiers from the product
No user-facing surface shows a price, tier name, comparison or upgrade CTA. /for-churches rewritten; /for-churches/demo, /docs/paid, tier-cards-section and pricing-calculator deleted with 308 redirects and a sitemap correction; /billing rewritten as Giving; /docs/churches, /docs/features and /help taken off tier language; the checkout union narrowed to donation so nobody can POST a price ID for a tier that no longer exists. Four routes unlocked (dashboard, care, team, groups) and three hidden behind notFound() until pj-s26-10 writes prayers.church_id, each stub keeping its sibling components rather than deleting work that will be needed again. billing.service.ts, billing.actions.ts, the Stripe webhook, plans.ts, mrr.ts and every billing DB column are retained deliberately for the Fable rebuild. Reviewed and approved 2026-07-26. Two findings recorded but not charged to this task: the /for-churches Prayer Team card is false and was authored by cc21686 (pj-s26-09), and chms-summary-scheduler gates on churches.currentPlan — a third gating mechanism the removal plan's limits.-scoped sweep was structurally incapable of finding, folded into pj-s27-08.
